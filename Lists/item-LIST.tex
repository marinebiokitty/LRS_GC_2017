...
\def\contain#1{%
  \s\MYcapacity{#1}%
  \append\MYmark{\mbox{ ~\MYvalue-VALUE~ }}}
...
\NEW{ItemLabel}{\iEngagementRings}{
...
\NEW{ItemLabel}{\iFancyUtilityBelt}{
...
\NEW{ItemLabel}{\iBulletproofCape}{
  ...
  \NEW{ItemLabel}{\iArtworkOne}{
  \s\MYname	{Magic Trick}
  \s\MYnumber	{0000}
  \s\MYtext	{Kinetic sculpture composed of spinning steel rounds, Worth 10K.}
  }
  
  \NEW{ItemLabel}{\iArtworkTwo}{
  \s\MYname	{To Incite}
  \s\MYnumber	{0000}
  \s\MYtext	{Masterful oil painting in bold strokes. Heavily inspired by the classical but look's to be fairly new. Worth 30K. }
    \s\MYcontents	{I was painted by someone special.}
  }
  
  \NEW{ItemLabel}{\iArtworkThree}{
  \s\MYname	{Map Key}
  \s\MYnumber	{0000}
  \s\MYtext	{A mesmerizing installation made of 1962 colored keys. Worth 20K.}
  ...
  \NEW{ItemLabel}{\iTinkerObjectOne}{
  \s\MYname	{VintageRecordPlayer}
  \s\MYnumber	{0000}
  \s\MYtext	{Fancy record player with large brass horn and legs, mother of pearl dials. Looks like it hasn't been used in a while. Worth \$5,000.}
  }
  
  \NEW{ItemLabel}{\iTinkerObjectTwo}{
  \s\MYname	{LavaLamp}
  \s\MYnumber	{0000}
  \s\MYtext	{Red and purple lava lamp. Work \$5,000}
  }
  
\NEW{ItemLabel}{\iTinkerObjectThree}{
  \s\MYname	{MassageChair}
  \s\MYnumber	{0000}
  \s\MYtext	{Comfortable-looking chair upholstered in black leather that reclines and massages. Worth \$10,000}
  }  
